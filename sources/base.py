...
# 積分欄提到泌尿即可，不再要求字面「泌尿科」。
#
# 🔴 **有界寫法，不可以直接放寬成 `"泌尿" in text`**：積分欄是自由文字，
# 「泌尿道感染衛教課程」這種正文詞會被當成積分（目前 459 列還沒出現，但遲早）。
# 所以「泌尿」後面必須緊接「科」、括號、或點數。
# 三種寫法都是實抓來的：`泌尿科(1點)`／`泌尿(3點)`／`泌尿：0.5點`。
_UROLOGY_CREDIT = re.compile(r"泌尿(?:科)?\s*[（(]|泌尿科|泌尿\s*[:：]?\s*\d")


def has_urology_credits(text: str) -> bool:
    """這筆有沒有掛泌尿科積分（含申請中）。這是本站的收錄門檻。

    2026-08-25 放寬：原本要求字面含「泌尿科」，漏掉了積分欄寫「泌尿(3點)」的場次
    （實抓 459 列中有 1 筆：高雄榮總 35 周年院慶暨台灣新創醫療學會半年會議）。

    🔴 **這道門同時扮演兩個角色，改的時候要知道自己在動哪一個**：
      ・角色 A：**擋年會的個別議程**（Podium／Symposium／海報，積分欄是空的）——
        實抓 2025-08 ～ 2027-04 共 459 列，其中 **258 列**是這種，全部靠
        「積分欄空白 → False」擋住。**`bool(text)` 那一段是這個角色的全部**。
      ・角色 B：判泌尿科相關性 —— 在這個來源上其實是多餘的：E-School 是 TUA 自己的
        積分登記表，實測「積分欄非空但完全沒提到泌尿」的列數 ＝ **0**，
        他科課程只有在同時申請泌尿科積分時才會出現。所以放寬角色 B 不會吸進他科課程。
    放寬的是角色 B，角色 A 一個字都沒動。

    🔴 **如果有人日後想把這道門整個拿掉**（例如「連沒積分的議程也要收」），
    必須先補一道跟積分無關的判準來接手角色 A，否則 258 筆議程會灌爆清單。
    可行的判準（已用實抓資料驗證過）：**積分欄空白且日期落在某場 kind=meeting
    事件的 date～end_date 區間內 → 判為議程，不獨立成筆**（258 列全命中、
    非年會日期的空積分列 ＝ 0，誤判率 0）；兜底是「同一天空積分列 ≥ 5 即視為大會日」
    （大會日實測最少 15 列，其餘 120 個有活動的日子都是 0，門檻 5 有 3 倍餘裕）。
    ⚠️ 這條現在**刻意沒有實作**：只要積分欄空白仍然回 False，258 列就被擋在門外，
    多做一道日期判準等於為零收益引入「來源執行順序依賴」這個新的故障維度。
    今天測起來沒差不是安全 —— 危險在 2027-01 半年會（約 35 筆）與 2027-08 年會
    （約 88 筆），那時若門已被拿掉才會爆。**判準寫在這裡，等真的要拆門時照著補。**
    """
    return bool(text) and bool(_UROLOGY_CREDIT.search(text))
